...
KEEPERS_COL_MAP = {
    ...
    'Player': 'player_name', 'Squad': 'squad', 'MP': 'appearances', 'CS': 'clean_sheets',
}

REQUIRED_STANDARD = ['player_name', 'squad', 'fbref_pos', 'appearances', 'minutes', 'goals', 'assists']
MIN_APPEARANCES   = 1
...
def normalize(df: pd.DataFrame, col_map: dict, required: list[str]) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"Missing after normalization: {missing} | Actual: {df.columns.tolist()}")
...
def load_all_seasons() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    return pd.concat(std_frames, ignore_index=True), pd.concat(gk_frames, ignore_index=True)
...
def transform():
    ...
    std_df['position'] = std_df.apply(lambda r: resolve_position(r['fbref_pos'], r['player_name']), axis=1)
    std_df['position_group'] = std_df['position'].map(POSITION_GROUPS)

    std_df = std_df.merge(
        gk_df.rename(columns={'squad': 'squad'}),
        on=['player_name', 'squad', 'season'], how='left'
    )
    std_df['clean_sheets'] = std_df['clean_sheets'].fillna(0).astype(int)

    GROUP_KEYS = ['player_name', 'club_slug', 'era_id', 'era_name', 'position', 'position_group']

    agg = std_df.groupby(GROUP_KEYS).agg(
    ...
    for club in CLUBS:
        ...
        for era in ERAS:
            era_seasons = {
                s: st for s, st in history.items()
            ...
            games = sum(
                (8 if season_start_year(s) >= 2024 else 6) + STAGE_GAMES[st]
                + (2 if season_start_year(s) >= 2024 and 'playoffs' in st.values else 0)
    ...
    output_cols = [
        'player_name', 'nationality', 'position', 'club_slug', 'era_id',
    ...
    print(f"Done: {len(agg)} player-era records, {len(club_era_rows)} club-era combinations")
